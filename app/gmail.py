...
import pickle
import os.path
from app.request import Request as Req
...
class GmailThread:
    ...
    def __repr__(self):
        return "<GmailThread numMessages=%r snippet=%r>" % (
            len(self.messages), self.snippet)
    ...
    def addLabel(self, label):
        ...
        # The global _addLabel() function implements this feature.
        _addLabel(self, label)

    def removeLabel(self, label):
        """Remove the label ``label`` from every message in this thread, if it's there."""
        # The global _removeLabel() function implements this feature.
        _removeLabel(self, label)

    def markAsRead(self):
        """Mark every message in this thread as read. (This does the same thing as removing the UNREAD label from the
        messages.)"""
        # The global _markAsRead() function implements this feature.
        _markAsRead(self)
    ...
    def markAsUnread(self):
        ...
        messages.)"""
        # The global _markAsUnread() function implements this feature.
        _markAsUnread(self)
...
class GmailMessage:
    ...
    def __init__(self, messageObj):
        ...
        if "parts" in messageObj["payload"].keys():
            for part in messageObj["payload"]["parts"]:
                if part["mimeType"].upper() == "TEXT/PLAIN" and "data" in part[
                    "body"]:
                ...
                if part["mimeType"].upper() == "MULTIPART/ALTERNATIVE":
                    ...
                    for multipartPart in part["parts"]:
                        if multipartPart[
                            "mimeType"].upper() == "TEXT/PLAIN" and "data" in \
                                multipartPart["body"]:
                            ...
                            # ``originalBody`` has the full body of the email, while the more useful ``body`` only has everything up until the quoted reply part.
                            self.originalBody = base64.urlsafe_b64decode(
                                multipartPart["body"]["data"]).decode(
                                emailEncoding
                            )
                            self.body = removeQuotedParts(self.originalBody)
                ...
                if "filename" in part.keys() and part["filename"] != "":
                    ...
                    self._attachmentsInfo.append(
                        {"filename": part["filename"], "id": attachmentId,
                         "size": attachmentSize}
                    )
    ...
    def downloadAttachment(self, filename, downloadFolder=".",
        ...
        try:
            attachmentIndex = \
                [i for i, v in enumerate(self.attachments) if v == filename][
                    duplicateIndex
        ...
        attachmentObj = (
            SERVICE_GMAIL.users()
                .messages()
                .attachments()
                .get(id=self._attachmentsInfo[attachmentIndex]["id"],
                     messageId=self.id, userId="me")
                .execute()
        )
    ...
    def downloadAllAttachments(self, downloadFolder=".", overwrite=True):
        ...
        if not overwrite:
            attachmentFilenames = [a["filename"] for a in
                                   self._attachmentsInfo]
        ...
        for attachmentInfo in self._attachmentsInfo:
            attachmentObj = (
                SERVICE_GMAIL.users()
                    .messages()
                    .attachments()
                    .get(id=attachmentInfo["id"], messageId=self.id,
                         userId="me")
                    .execute()
            )
    ...
    def addLabel(self, label):
        """Add the label ``label`` to every message in this thread."""
        _addLabel(self,
                  label)  # The global _addLabel() function implements this feature.

    def removeLabel(self, label):
        """Remove the label ``label`` from every message in this thread, if it's there."""
        _removeLabel(self,
                     label)  # The global _removeLabel() function implements this feature.
    ...
    def reply(self, body, attachments=None, cc=None, bcc=None,
              mimeSubtype="plain"):
        """Like the send() function, but replies to the last message in this thread."""
        ...
        return send(self.sender, self.subject, body, attachments=attachments,
                    cc=cc,
                    bcc=bcc, mimeSubtype=mimeSubtype, _threadId=self.threadId)

    def replyAll(self, body, attachments=None, cc=None, bcc=None,
                 mimeSubtype="plain"):
        """Like the send() function, but replies to the last message in this thread."""
...
def init(userId="me", tokenFile="token.pickle",
         credentialsFile="credentials.json", _raiseException=True):
    # 还原路径
    root_path = f'{os.getcwd()}/conf/'
    credentialsFile = f'{root_path}{credentialsFile}'
    tokenFile = f'{root_path}{tokenFile}'
    ...
    try:
        ...
        if not creds or not creds.valid:
            ...
            if creds and creds.expired and creds.refresh_token:
                ...
                # 调用初始化http的参数
                flow.oauth2session.proxies = session.proxies
                creds = flow.run_local_server(port=0, open_browser=False)
            # Save the credentials for the next run
            with open(tokenFile, 'wb') as token:
                pickle.dump(creds, token)
        # AuthorizedHttp实例| 初始化一个http
        authorized_http = google_auth_httplib2.AuthorizedHttp(
            credentials=creds, http=Req().init_httplib2())
        # http = google_auth_httplib2.AuthorizedHttp(credentials=creds,
        #                                            http=build_http())
        # http = google_auth_httplib2.AuthorizedHttp(credentials, http=build_http())
        SERVICE_GMAIL = build("gmail", "v1", http=authorized_http)

        EMAIL_ADDRESS = \
            SERVICE_GMAIL.users().getProfile(userId=userId).execute()[
                "emailAddress"]
...
def _createMessage(sender, recipient, subject, body, cc=None, bcc=None,
                   mimeSubtype="plain", _threadId=None):
...
def _createMessageWithAttachments(sender, recipient, subject, body,
                                  attachments, cc=None, bcc=None,
                                  mimeSubtype="plain", _threadId=None):
    """Creates a MIMEText object and returns it as a base64 encoded string in a ``{'raw': b64_MIMEText_object}``
    ...
    if isinstance(attachments, str):
        attachments = [
            attachments]  # If it's a string, put ``attachments`` in a list.
    ...
    for attachment in attachments:
        ...
        mimePart.add_header("Content-Disposition", "attachment",
                            filename=filename)
...
def _sendMessage(message, userId="me"):
    ...
    message = SERVICE_GMAIL.users().messages().send(userId=userId,
                                                    body=message).execute()
    return message


def send(recipient, subject, body, attachments=None, sender=None, cc=None,
         bcc=None, mimeSubtype="plain", _threadId=None):
    ...
    if attachments is None:
        msg = _createMessage(sender, recipient, subject, body, cc, bcc,
                             mimeSubtype, _threadId=_threadId)
    else:
        msg = _createMessageWithAttachments(sender, recipient, subject, body,
                                            attachments, cc, bcc, mimeSubtype,
                                            _threadId=_threadId)
...
def _removeLabel(gmailObjects, label, userId="me"):
    ...
    for obj in gmailObjects:
        if isinstance(obj, GmailThread):
            SERVICE_GMAIL.users().threads().modify(userId=userId, id=obj.id,
                                                   body=removeUnreadLabelObj).execute()
        elif isinstance(obj, GmailMessage):
            SERVICE_GMAIL.users().messages().modify(userId=userId, id=obj.id,
                                                    body=removeUnreadLabelObj).execute()
...
def _trash(gmailObjects, userId="me"):
    ...
    if isinstance(gmailObjects, (GmailThread, GmailMessage)):
        ...
        for obj in gmailObjects:
            if isinstance(obj, GmailThread):
                SERVICE_GMAIL.users().threads().trash(userId=userId,
                                                      id=obj.id).execute()
            elif isinstance(obj, GmailMessage):
                SERVICE_GMAIL.users().messages().trash(userId=userId,
                                                       id=obj.id).execute()
